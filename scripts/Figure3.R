...
plot_AIC_distribution <- function( )
{
  ...
  ### AIC DISTRIBUTIONS ###
  p = ggplot(data, aes(x=Model, y=AIC)) +
    scale_fill_brewer(palette = "BrBG") +
    scale_color_brewer(palette = "BrBG") +
    geom_violin(aes(fill=Model), alpha=0.6) +
    geom_boxplot(width=0.1) +
    annotate(geom="text", x=unique(data$Model), y=c(352,369,336,356), label=c("a", "b", "c", "a"), fontface=2, size=5) +
    #stat_compare_means(comparisons=my_comparisons) +
    xlab("Model") +
    ylab("AIC distribution (100 repetitions)") +
    ggtitle("AIC distribution of each calibrated model") +
    theme_classic() +
    theme(axis.text.x=element_blank())
  #p2$layers[[2]]$aes_params$textsize = 1.5
  return(p)
}
...
p = plot_AIC_distribution()
ggsave("figures/Figure3.pdf", p, width=7, height=5, units="in")
