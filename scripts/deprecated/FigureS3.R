...
### Plot score distribution through models ###
plot_score_with_letters <- function( data, my_comparisons, score_name, score_label, letters, letters_pos )
{
  ...
  p = ggplot(data, aes_string(x="Model", y=score_name, fill="Model")) +
    scale_fill_brewer(palette = "BrBG") +
    geom_boxplot() +
    annotate(geom="label", x=unique(data$Model), y=rep(letters_pos, 4), label=letters, fontface=2, size=letters_size, label.size=NA) +
    xlab("Model") +
    ylab(paste0(score_label, "\n(100 repetitions)")) +
    ggtitle(score_label) +
    theme_classic() +
...
plot_evaluation_metrics_with_letters <- function()
{
  ...
  ###############################
  letters = c("a", "b", "c", "c")
  p1      = plot_score_with_letters(data, my_comparisons, "AUC", "AUC", letters, 0.835)
  ###############################
  letters = c("a", "b", "c", "d")
  p2      = plot_score_with_letters(data, my_comparisons, "Sensitivity", "Sensitivity", letters, 1.15)
  ###############################
  letters = c("a", "a", "b", "a")
  p3      = plot_score_with_letters(data, my_comparisons, "Specificity", "Specificity", letters, 1.15)
  ###############################
  letters = c("a", "b", "c", "d")
  p4      = plot_score_with_letters(data, my_comparisons, "F1", "F1 score", letters, 0.55)
  ###############################
  letters = c("a", "a", "b", "c")
  p5      = plot_score_with_letters(data, my_comparisons, "KAPPA", "Standard Kappa", letters, 0.45)
  ###############################
  letters = c("a", "b", "c", "d")
  p6      = plot_score_with_letters(data, my_comparisons, "TSS", "True skill statistic", letters, 0.60)
  ###############################
  p = ggarrange(p1, p2, p3, p4, p5, p6, ncol=2, nrow=3, common.legend = TRUE, legend="bottom", labels=c("(a)","(b)","(c)","(d)","(e)","(f)"))
  return(p)
}
...
p = plot_evaluation_metrics_with_p_values()
ggsave("figures/FigureS3.pdf", p, width=7, height=8, units="in")
